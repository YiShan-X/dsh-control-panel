 ...
 *
 * Only the subset npm actually publishes is accepted: build metadata is
 * tolerated and ignored, anything else returns null so the caller can say
 * "I could not compare these" rather than guessing an order.
 *
 * @param {string} input
 ...
 *
 * The prerelease rules are the whole reason this exists rather than a string
 * comparison: DSH publishes `0.1.5-rc.2` and `0.1.6-alpha.2`, and the question
 * the panel must answer is whether the second is newer. A string compare says
 * "0.1.5-rc.2" > "0.1.6-alpha.2" (because "5" > "6" is false, but "rc" > "a"),
